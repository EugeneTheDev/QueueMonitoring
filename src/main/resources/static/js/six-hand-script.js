    ...
    var openPlace = function(place, coordinates){
        $.ajax({
            ...
            success:function(data){
                ...
                + data.peopleInQueue + 
                ' <i class="fas fa-male fa-lg icon"></i> </p> </div> </div> <div class="col-xs-12 col-md-3"> <button class="right">Я на месте</button> </div> </div> <br> <form action=""> <input type="number" name="num" value="aa" placeholder="Человек в очереди"> <button class="change">Изменить</button> </form> </div> </div> </div> </div> </div>'
